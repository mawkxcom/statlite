...
const trackHandler: RequestHandler = (req, res) => {
  const ip = getClientIp(req.headers as any, req.socket.remoteAddress);
  ...
  enqueueWrite(() => {
    ...
    db.prepare(
      'INSERT INTO visitors(id, ip, ua, first_seen, last_seen) VALUES(?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET last_seen=excluded.last_seen'
    ).run(visitorId, ip, ua, now, now);

    const recent = db
      .prepare(
        'SELECT 1 FROM page_views WHERE site_id=? AND page_path=? AND visitor_id=? AND created_at>? LIMIT 1'
      )
      .get(site, page, visitorId, now - 30_000);
    if (!recent) {
      db.prepare(
        'INSERT INTO page_views(site_id, page_path, visitor_id, ip, ua, created_at) VALUES(?,?,?,?,?,?)'
      ).run(site, page, visitorId, ip, ua, now);
    }
  });

  // Stats snapshot
  const totalPvRow = db.prepare('SELECT COUNT(*) as c FROM page_views WHERE site_id=?').get(site) as { c: number };
  const totalPv = Number(totalPvRow?.c ?? 0);
  const totalUv = db
    .prepare('SELECT COUNT(DISTINCT visitor_id) as c FROM page_views WHERE site_id=?')
    .get(site) as { c: number };
  const totalUvNum = Number(totalUv?.c ?? 0);
  const pagePv = db
    .prepare('SELECT COUNT(*) as c FROM page_views WHERE site_id=? AND page_path=?')
    .get(site, page) as { c: number };
...
statsRouter.get('/summary', ((req, res) => {
  ...
  const totalPvRow2 = db.prepare('SELECT COUNT(*) as c FROM page_views WHERE site_id=?').get(site) as { c: number };
